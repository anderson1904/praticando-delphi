...
//procedimentos de exclusão
  ...
  begin
//    if FClientes.IndexOf(aCliente) = -1 then
//      raise Exception.Create('não tem esse cliente na lista');
//    FClientes.Remove(aCliente);
  end;

  procedure TBanco.DesativarConta(aConta: TConta);
  begin
//    if FContas.IndexOf(aConta) = -1 then
//      raise Exception.Create('não tem essa conta na lista');
//    FContas.Remove(aConta);
  end;
